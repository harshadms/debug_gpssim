 ...
 */
void computeRange(range_t *rho, ephem_t eph, ionoutc_t *ionoutc, gpstime_t g, double xyz[], int prn)
{
			...
			if (allocatedSat[sv]==-1) // Visible but not allocated
			{
				...
				for (i=0; i<MAX_CHAN; i++)
				{
					if (chan[i].prn==0)
					{
						...
						computeRange(&rho, eph[sv], &ionoutc, grx, xyz, chan[i].prn);
						chan[i].rho0 = rho;

						// Initialize carrier phase
						r_xyz = rho.range;

						computeRange(&rho, eph[sv], &ionoutc, grx, ref, chan[i].prn);
						r_ref = rho.range;
	...
	for (iumd=1; iumd<numd; iumd++)
	{
		for (i=0; i<MAX_CHAN; i++)
		{
			if (chan[i].prn>0)
			{
				...
				// Current pseudorange
				if (!staticLocationMode)
					computeRange(&rho, eph[ieph][sv], &ionoutc, grx, xyz[iumd], chan[i].prn);
				else
					computeRange(&rho, eph[ieph][sv], &ionoutc, grx, xyz[0], chan[i].prn);
